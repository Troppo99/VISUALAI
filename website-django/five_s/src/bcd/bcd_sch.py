...
from bcd_core import BroCarpDetector
...
class Scheduling:
    ...
    def start_detection(self):
        ...
        if not self.detector:
            print("Starting BroCarpDetector...")
            self.detector = BroCarpDetector(**self.detector_args)
            detection_thread = threading.Thread(target=self.detector.main)
            detection_thread.daemon = True
            detection_thread.start()
        else:
            print("BroCarpDetector is already running.")

    def stop_detection(self):
        if self.detector:
            print("Stopping BroCarpDetector...")
            self.detector.stop_event.set()
            self.detector = None
        else:
            print("BroCarpDetector is not running.")
    ...
    def shutdown(self):
        print("Shutdown scheduler and BroCarpDetector if not running...")
